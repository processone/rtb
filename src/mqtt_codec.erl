...
decode_reason_code(suback, Code) -> decode_suback_code(Code);
decode_reason_code(unsuback, Code) -> decode_unsuback_code(Code);
...
encode_prop(assigned_client_identifier, ID) when ID /= <<>> ->
    <<18, (size(ID)):16, ID/binary>>;
encode_prop(authentication_data, Data) when Data /= <<>> ->
    <<22, (size(Data)):16, Data/binary>>;
encode_prop(authentication_method, M) when M /= <<>> ->
    <<21, (size(M)):16, M/binary>>;
encode_prop(content_type, T) when T /= <<>> ->
...
encode_prop(request_response_information, true) ->
    <<25, 1>>;
encode_prop(response_information, S) when S /= <<>> ->
...
%%% Validators
%%%===================================================================
-spec assert(T, any(), error_reason()) -> T.
assert(Got, Got, _) ->
    Got;
