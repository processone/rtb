...
handle_info({gun_ws, _, _, {text, Text}}, StateName, State) ->
    lager:warning(
      "Unexpected UTF-8 frame:~n~p~n** in state ~s:~n~s",
      [Text, StateName, pp(State)]),
    stop(StateName, State, {socket, eproto});
handle_info({gun_ws, _, _, _}, StateName, State) ->
    stop(StateName, State, {socket, closed});
handle_info({gun_down, _, _, _, _, _}, StateName, State) ->
    stop(StateName, State#state{socket = undefined}, {socket, closed});
handle_info({gun_error, _, _}, StateName, State) ->
    stop(StateName, State#state{socket = undefined}, {socket, closed});
handle_info({gun_error, _, _, _}, StateName, State) ->
    stop(StateName, State#state{socket = undefined}, {socket, closed});
